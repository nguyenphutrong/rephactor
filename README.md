...
seeded PHP internal functions and constants, PHP internal class/interface
symbols, PHP superglobals, static methods, class constants, and static
...
PHP internal method completions for the seeded internal class set. It also
includes common PHP keyword completions and adds a `use`, `use function`, or
`use const` declaration edit for unambiguous namespaced class, function, and
constant completions when the short name is not already imported or shadowed.
...
that import them. Method implementations show indexed parent/interface
declaration counts when available.
...
literals, match expressions, imports, heredoc/nowdoc strings, comments, and
...
`dirname(__DIR__)` prefixes with optional dirname levels and
`DIRECTORY_SEPARATOR`, to files on disk.
...
- PHP internal class/interface symbol completions.
...
- Unresolved class-level PHPDoc `@property`, `@property-read`,
  ...
  documents.
- Unresolved local PHPDoc `@var` type diagnostics for open PHP documents.
...
- Folding ranges for PHP blocks, array literals, match expressions, imports,
...
- Whole-document and range whitespace formatting for trailing whitespace; whole
  ...
  placement for class/function/control blocks, then normalizes indentation by
  brace depth.
...
- PHPDoc parameter-type inlay hints for untyped function-like parameters,
...
- Document links for literal relative include/require paths, concatenated
  ...
  prefixes with optional dirname levels and `DIRECTORY_SEPARATOR`.
...
- Function-like PHPDoc `@param Type $variable` annotations for instance method
  resolution.
- Function-like PHPDoc `@return self`, `@return static`, and direct
  `@return parent` annotations for conservative return diagnostics.
...
- A seed set of PHP internal functions, such as `str_replace`, `json_encode`,
  `json_decode`, `json_last_error`, `json_last_error_msg`, `class_exists`,
  `function_exists`, `interface_exists`, `method_exists`, `property_exists`, `trait_exists`,
  ...
  `array_fill_keys`, `array_intersect`, `array_pad`, `array_product`, `array_rand`,
  `array_reduce`, `array_replace`, `array_replace_recursive`, `array_reverse`,
  `array_unique`, `asort`, `ksort`, `rsort`, `sort`, `date`, `strtotime`,
  `array_sum`, `array_walk`, `array_walk_recursive`, `compact`, `extract`, `range`, `shuffle`,
  ...
  `extension_loaded`, `getenv`, `ini_get`, `ini_set`, `memory_get_peak_usage`,
  `memory_get_usage`, `phpversion`, `putenv`, `version_compare`,
