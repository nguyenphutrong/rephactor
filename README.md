...
tag lines when available. Hover for seeded PHP internal functions and PHP
internal class/interface symbols links to the official PHP manual, and seeded
PHP internal constants show concise constant labels. Seeded internal functions
include conservative parameter and return metadata for simple scalar/array
contracts. It intentionally avoids rich PHPDoc rendering and
...
`__DIR__` and `dirname(__FILE__)` concatenated literals, to files on disk.
...
- Document links for literal relative include/require paths and `__DIR__` or
  `dirname(__FILE__)` concatenated literals.
...
- A seed set of PHP internal functions, such as `str_replace`, `json_encode`,
  ...
  `preg_match_all`, `preg_split`, `htmlspecialchars`, `mb_strlen`, and
